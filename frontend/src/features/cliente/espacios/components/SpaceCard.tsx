...
export default function SpaceCard({ space }: SpaceCardProps) {
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-2xl border border-[#e7e8f2] bg-white">
      <div className="relative flex h-[130px] items-center justify-center overflow-hidden bg-[#f4f5fc]">
        {space.imageUrl ? (
          <img
            src={space.imageUrl}
            alt={space.title}
      ...
      <div className="flex flex-col gap-3 p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          {space.reviewCount > 0 ? (
            ...
            <span className="shrink-0 font-['Quicksand',sans-serif] text-xs text-[#a098ae]">
              Sin reseñas
            </span>
          )}
        </div>

        <p className="font-['Quicksand',sans-serif] text-sm text-[#8b899e]">{space.location}</p>
